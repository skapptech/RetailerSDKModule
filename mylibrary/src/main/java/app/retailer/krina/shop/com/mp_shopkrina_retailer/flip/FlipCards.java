...
public class FlipCards {
    ...
    FlipCards(FlipViewController controller, boolean orientationVertical) {
    ...
    boolean isFirstDrawFinished() {
        return firstDrawFinished;
    }

    void setFirstDrawFinished(boolean firstDrawFinished) {
    ...
    void reloadTexture(int frontIndex, View frontView, int backIndex, View backView) {
        synchronized (this) {
            boolean
                    frontChanged =
    ...
    synchronized void draw(FlipRenderer renderer, GL10 gl) {
            ...
            case MotionEvent.ACTION_MOVE:
                ...
                if (state == STATE_TOUCH) {
                    ...
                    if (frontCards.getIndex() == maxIndex - 1) { //the last page
                        if (accumulatedAngle > frontCards.getIndex() * 180) {
                            accumulatedAngle = Math.min(accumulatedAngle,
                                    controller.isOverFlipEnabled() ? (frontCards.getIndex() * 180
                                            + MAX_TIP_ANGLE) : (frontCards.getIndex() * 180));
                        }
                    }
