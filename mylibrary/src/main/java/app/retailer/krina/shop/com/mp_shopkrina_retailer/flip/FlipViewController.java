...
public class FlipViewController extends AdapterView<Adapter> {
    ...
    private final LinkedList<View> releasedViews = new LinkedList<>(); //XXX: use a SparseArray to keep the related view indices?
    ...
    void postFlippedToView(final int indexInAdapter) {
        handler.post(() -> flippedToView(indexInAdapter, true));
    }
    ...
    void postHideFlipAnimation() {
        if (inFlipAnimation) {
            handler.post(this::hideFlipAnimation);
        }
    }
