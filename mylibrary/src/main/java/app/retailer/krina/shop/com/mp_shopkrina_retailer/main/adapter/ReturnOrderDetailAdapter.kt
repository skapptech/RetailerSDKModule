...
class ReturnOrderDetailAdapter(
    ...
    override fun onBindViewHolder(holder: ViewHolder, position: Int) {
        ...
        holder.mBinding.tvPrice.text =
            RetailerSDKApp.getInstance().dbHelper.getString(R.string.item_mrp) + "  " + model.getPrice()
        if (model.getRequestType() == 0) {
            holder.mBinding.tvQty.text =
                RetailerSDKApp.getInstance().dbHelper.getString(R.string.text_return_qty) + " " + model.getQty()
        } else {
            holder.mBinding.tvQty.text =
                RetailerSDKApp.getInstance().dbHelper.getString(R.string.text_return_qty) + " " + model.getQty()
        }
