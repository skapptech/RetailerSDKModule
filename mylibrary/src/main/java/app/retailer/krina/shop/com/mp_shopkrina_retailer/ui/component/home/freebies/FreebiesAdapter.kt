...
class FreebiesAdapter(
    ...
    override fun onBindViewHolder(viewHolder: ViewHolder, position: Int) {
        ...
        viewHolder.tvMultiMoq.setOnClickListener { v: View? ->
            ...
            val listener = AdapterInterface { pos ->
                val moq = list!![position].moqList
                list!![position] = list!![position].moqList[pos]
                list!![position].moqList = moq
                for (j in list!![position].moqList.indices) {
                    list!![position].moqList[j].isChecked = pos == j
                }
                list!![position].moqList[pos].isChecked = true
                notifyDataSetChanged()
        ...
        try {
            ...
            if (model.isOffer) {
                ...
                when (model.offerType) {
                    "WalletPoint" -> {
                        // String sfreewalletDP = new DecimalFormat("##.##").format(moqPojoArrayList.get(0).getOfferWalletPoint());
                    ...
                    "ItemMaster" -> {
                        // viewHolder.freeItemQutTV.setText(String.valueOf(moqPojoArrayList.get(0).getOfferFreeItemQuantity()));
        ...
        try {
                ...
                try {
                    if (model.offerType != null) {
                        if (model.offerType.equals("FlashDeal", ignoreCase = true)) {
                            ...
                            if (model.flashDealMaxQtyPersonCanTake >= itemQuantity + model.minOrderQty && model.offerQtyAvaiable > itemQuantity) {
                                flashDealFlag = false
                            } else {
                                if (!isflashDealUsed) {
                                    flashDealFlag = true
            ...
            if (addFlag) {
                ...
                addToCartInterface.addToCart(
                    model.itemId, itemQuantity, calUnitPrice, model,
    ...
    inner class ViewHolder(var mBinding: CategoryOrderItemsBinding) : RecyclerView.ViewHolder(
        ...
        var relItemDetails: RelativeLayout
